...
class LayerCategoryManager extends BaseEntityManager implements LayerCategoryManagerInterface {
     ...
     * @return LayerCategoryInterface[]
     */
    public function getCategoryLayers(LayerCategoryInterface $category) {
        if ($category === null) {
            return null;
        }
        $result_layers = array();
        $layers = $category->getLayers();
        foreach ($layers as $layer) {
            if ($this->user) {
                $isShared = $this->inSharedGroup($this->user->getGroups(), $layer->getGroups());

                if (($layer->enabled === true && $layer->public === true) || ( $layer->enabled === true && $layer->user === $this->user) || ( $layer->enabled === true && $isShared)) {
                    array_push($result_layers, $layer);
                }
            } else {
                if ($layer->enabled === true && $layer->public === true) {
                    array_push($result_layers, $layer);
                }
            }
        }
    ...
    private function inSharedGroup($usergroups, $layergroups) {
        if ($usergroups === null || $layergroups === null) {
            return false;
        }
        foreach ($usergroups as $usergroup) {
            foreach ($layergroups as $layergroup) {
                if ($layergroup === $usergroup) {
                    return true;
                }
            }
        }
