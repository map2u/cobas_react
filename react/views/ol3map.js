...
import {LayerList} from '../components/react-openlayers/custom/LayerList';

import { Sidebar, Tab } from '../components/react-ol-sidebarv2/index';
...
} from "../components/react-openlayers/index";
...
class Map2uOl3map extends React.Component<any, any>  {
  ...
  fetchOverlays() {
      ...
      fetch('http://localhost:8040/layerlist').then(function (response) {
        return response.json();
      }).then(function (result) {
        that.setState({ overlays: result.data.layerlist });
      //  console.log(that.state.overlays);
      });
 ...
 // console.log(this.state.overlays);
    return (
      <div>
        <Sidebar id="sidebar" ref='ol3map_sidebar' closeIcon ={this.state.closeIcon} collapsed={this.state.collapsed} position={this.state.postion} selected={this.state.selected}
          ...
          <Tab id="home" header="Layers" icon="fa fa-map-o">
            <LayerList overlays={this.state.overlays}/>
          </Tab>
          <Tab id="layerlist_tree" header="Feature Information" icon="fa fa-home">
  ...
  componentWillUnmount() {
    ...
    if(typeof this.setTarget ==='function') {
